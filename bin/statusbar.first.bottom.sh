...
VOL_TOGGLE_CMD="pulseaudio-ctl mute"
VOL_UP_CMD="pulseaudio-ctl up"
VOL_DOWN_CMD="pulseaudio-ctl down"
...
PULSE_START_CMD="pulseaudio --start"
PULSE_END_CMD="pulseaudio -k"
...
printTempInfo() {
        CPUTemp0=$(sensors | grep "temp2" | awk '{print $2}')
        GPUTemp0=$(nvidia-settings -q gpucoretemp -t | tail -1)
...
printDiskInfo() {
	...
	HFSP=$(df -h /home | tail -1 | awk '{ print $4 }' | sed 's/G/ GB/')
...
printPulseAudioStatus() {
	PulseAudioON=$(ps -A | grep -c pulseaudio)
	if [[ $PulseAudioON == "0" ]]; then
                echo -n "^fg($DZEN_FG2)^ca(1,$PULSE_START_CMD)^i("$ICONPATH"player_stop1.xbm)^ca() ^fg()PA "
	else
                echo -n "^fg($DZEN_FG2)^ca(1,$PULSE_END_CMD)^i("$ICONPATH"player_stop1.xbm)^ca() ^fg($OKCOLOR)PA "
	fi
	return
}
...
printMpdInfo() {
	...
	if [[ $MPDON == "0" ]]; then
		...
		#echo -n "^fg($DZEN_FG2)| ^ca(1,$MPD_RAND_CMD)RANDOM^ca() ^fg()$MpdRandom "
           ...
           echo -n "^fg($DZEN_FG2)^ca(1,"${HOME}/bin/dzen/dzen_music.sh")^i("$ICONPATH2"note.xbm) ^ca(3,$MPD_TOGGLE_CMD)$MpdInfo^ca()^ca()"
	fi
	return
}
