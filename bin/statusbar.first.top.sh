...
printDateInfo() {
        echo -n "^i("$ICONPATH"clock2.xbm) ^ca(1,$CAL_CMD)^ca(3,$GCAL_CMD)^fg()$(date '+%a %B^fg(#444) ^fg(#3955c4)%d^fg(#444), ^fg()%Y^fg(#3955c4) ^fg(#363636)- ^fg()%H^fg(#444):^fg()%M^fg(#444)^fg()')^ca()^ca() "
	return
}
...
printTopBar | dzen2 -x $X_POS -y $Y_POS -w $WIDTH -h $HEIGHT -fn $FONT -ta 'r' -bg $DZEN_BG -fg $DZEN_FG -p -e ''
#tray
