...
(echo "^fg($foreground)Music"; echo -e "^fg($cyan0)Now playing\n\n^fg($foreground)^ca(1,$ARTIST_CMD)$artist^ca()\n^fg($blue1)^ca(1,$LYRICS_CMD)$track^ca()\n^fg($cyan1)^ca(1,$ALBUM_CMD)$album^ca()"; echo " "; echo "^ca(1,$NEXT_CMD)Next: ^fg($foreground)$next^ca()"; echo "^ca(1,$PREV_CMD)Prev: ^fg($foreground)$prev^ca()"; echo " "; echo "$stats"; sleep 15) | dzen2 -bg $background -fn $FONT -x $XPOS -y $YPOS -w $WIDTH -l $LINES -sa c -e 'onstart=uncollapse,hide;button1=exit;button3=exit'
